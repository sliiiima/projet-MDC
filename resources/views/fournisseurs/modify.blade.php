@extends('layout.app')
@section('mini title', 'Modifier un Fournisseurs')
@section('content')
    <form action="{{ route('fournisseurs.update',$fournisseur->id) }}" method="post">
        ...
        <div class="grid gap-6 mb-6 md:grid-cols-3">
            <div>
                ...
                <select id="medicament_id" name="medicament_id"
                    ...
                    @foreach (\App\Models\Medicament::all() as $medicament)
                        <option value="{{ $medicament->id }}" @if($fournisseur->medicament_id == $medicament->id) selected @endif>{{ $medicament->nom }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <div class="relative max-w-sm w-full">
                    ...
                    <input id="datepicker-autohide" name="date_recue" value="{{ $fournisseur->date_recue }}" datepicker datepicker-autohide datepicker-format="yyyy-mm-dd" type="text"
            ...
            <div>
                ...
                <input type="text" id="qte_recue" name="qte_recue" value="{{ $fournisseur->qte_recue }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 "
